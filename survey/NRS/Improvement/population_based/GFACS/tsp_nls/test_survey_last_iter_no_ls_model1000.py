...
# ! 最后一代去掉ls，只输出最后一代最优的版本
@torch.no_grad()
def infer_instance_without_ls_last(model, pyg_data, distances, n_ants, t_aco_diff):
    if model is not None:
        model.eval()
        heu_vec = model(pyg_data)
        heu_mat = model.reshape(pyg_data, heu_vec) + EPS
    ...
    results = torch.zeros(size=(len(t_aco_diff),))
    total_elapsed_aco = 0.0

    for i, t in enumerate(t_aco_diff):
        t_start = time.time()  # ---记录本段 ACO 开始时间---

        if i < len(t_aco_diff) - 1:
            # 前几段：照常 run（含局部搜索）
            cost, _, _ = aco.run(t, start_node=START_NODE)
        else:
            # 最后一段：不做 LS，并记录 best_path
            cost, _, _ = aco.run_without_ls_last(t, start_node=START_NODE)

        # cost, _, _ = aco.run(t, start_node=START_NODE)

        t_end = time.time()    # ---结束时间---
        results[i] = cost
        total_elapsed_aco += (t_end - t_start)

    # 返回路径与 ACO 运行总耗时
    return results, aco.shortest_path, total_elapsed_aco
...
def main(ckpt_path, k_sparse_factor=10, n_ants=100, n_iter=10, guided_exploration=False, seed=0):
    # ---------- logger ----------
    result_root = './tsp_nls/survey_results_last_without_ls'
    ...
    for h in handles:
        ...
        try:
            ...
            # 2) 推理
            _, path, _ = infer_instance_without_ls_last(net, pyg_data, dist_int, n_ants, t_aco_diff)
...
if __name__ == "__main__":
    # 固定参数
    ckpt_path = "./pretrained/tsp_nls/1000/tsp1000_sd0_fromckpt-500_tsp500_sd0_50/20.pt"  # 模型文件路径，或者 None
    # n_nodes = 200           # TSP 节点数
