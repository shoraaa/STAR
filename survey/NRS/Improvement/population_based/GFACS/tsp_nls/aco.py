...
class ACO():
    def __init__(
        ...
        local_search_type: Optional[str] = 'nls',
        device='cpu',
    ):
    ...
    # ! 新增：最后一代不做局部搜索
    @torch.no_grad()
    def run_without_ls_last(self, n_iterations, start_node=None):
        """
        与 run 保持一致的流程，但最后一代不做 local search；
        返回“最后一代纯采样”的 best_cost、diversity、耗时与 best_path，
        并将该最优路径赋给 self.shortest_path（同时更新 self.lowest_cost）。
        """
        assert n_iterations > 0
        ...
        for it in range(n_iterations):
            # 1) 概率矩阵
            probmat = (self.pheromone ** self.alpha) * (self.heuristic ** self.beta)
            # 2) 纯采样（先 numpy 后转 torch）
            paths_np = numba_sample(probmat.cpu().numpy(), self.n_ants, start_node=start_node)
            paths = torch.from_numpy(paths_np.T.astype(np.int64)).to(self.device)
            _paths = paths.clone()  # 用于Jaccard多样性计算

            if it < n_iterations - 1:
                # 非最后一代：和 run 完全一致（含局部搜索）
                paths_ls = self.local_search(paths, inference=True)
                costs = self.gen_path_costs(paths_ls)

                best_cost, best_idx = costs.min(dim=0)
                if best_cost < self.lowest_cost:
                    self.shortest_path = paths_ls[:, best_idx]
                    self.lowest_cost = best_cost.item()

                self.update_pheromone(paths_ls, costs)

            else:
                # 最后一代：不做局部搜索
                last_costs = self.gen_path_costs(paths)

                # 与 run 的更新顺序一致：先用本代路径更新信息素
                self.update_pheromone(paths, last_costs)

                # 计算本代（纯采样）多样性
                edge_sets = []
                _paths_np = _paths.T.cpu().numpy()
                for _p in _paths_np:
                    edge_sets.append(set(map(frozenset, zip(_p[:-1], _p[1:]))))
                jaccard_sum = 0.0
                for i, j in combinations(range(len(edge_sets)), 2):
                    jaccard_sum += len(edge_sets[i] & edge_sets[j]) / len(edge_sets[i] | edge_sets[j])
                diversity = 1 - jaccard_sum / (len(edge_sets) * (len(edge_sets) - 1) / 2) if len(edge_sets) >= 2 else 0.0

                # 取最后一代的最优路径，并赋值到 self.shortest_path（同步 lowest_cost）
                best_idx = last_costs.argmin().item()
                best_path = paths[:, best_idx]
                last_best = last_costs[best_idx].item()
                self.shortest_path = best_path                      # 关键：供 infer_instance_without_ls_last 使用
                self.lowest_cost = last_best                        # 同步值，避免后续依赖不一致

                end = time.time()
                return last_best, diversity, end - start
