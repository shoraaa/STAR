...
class ACO():
    ...
    @torch.no_grad()
    def run_lastgen_no_ls(self, n_iterations: int):
        """
        与原 run 的差异：
        1) 仅最后一代不进行 local search；
        2) self.shortest_path / self.lowest_cost 仅依据 “最后一代” 的解来设置。
        返回：(lowest_cost_on_last_gen, diversity_on_last_gen_preLS, elapsed_time)
        """
        assert n_iterations > 0

        start = time.time()
        lastgen_paths_preLS = None  # 用于最后一代的多样性（保持与原实现一致：在 LS 前统计边集）
        lastgen_costs = None

        for it in range(n_iterations):
            # 生成一批路径（构造解）
            paths = self.gen_path(require_prob=False)
            paths_preLS = paths.clone()

            # 仅非最后一代执行 local search；最后一代严格不做 LS
            if it < n_iterations - 1 and self.local_search_type is not None:
                self.multiple_swap_star(paths, inference=True)

            # 计算当前代（可能已 LS）的成本
            costs = self.gen_path_costs(paths)

            # 仍然按 ACO 逻辑进行信息素更新（包括最后一代）
            self.update_pheromone(paths, costs)

            # 只在“最后一代”设置 shortest_path / lowest_cost，并保留多样性统计用的 pre-LS 路径
            if it == n_iterations - 1:
                best_cost, best_idx = costs.min(dim=0)
                self.shortest_path = paths[:, best_idx].clone()
                self.lowest_cost = best_cost.item()
                lastgen_paths_preLS = paths_preLS
                lastgen_costs = costs

        end = time.time()

        # --- 多样性：沿用原实现口径，在（最后一代）的“局部搜索前”路径上统计 ---
        edge_sets = []
        _paths = lastgen_paths_preLS.T.cpu().numpy()  # (n_ants, seq_len)
        for _p in _paths:
            edge_sets.append(set(map(frozenset, zip(_p[:-1], _p[1:]))))
        ...
        denom = (len(edge_sets) * (len(edge_sets) - 1) / 2) if len(edge_sets) >= 2 else 1
        diversity = 1 - jaccard_sum / denom
