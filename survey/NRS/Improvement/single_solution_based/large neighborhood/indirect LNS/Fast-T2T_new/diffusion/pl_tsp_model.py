...
from diffusion.utils.tsp_utils import apply_tsplib_rounding
from diffusion.co_datasets.tsp_tsplib_dataset import TSPTSPLIBDataset  # ← 新增
...
class TSPModel(COMetaModel):
    ...
    def __init__(self, param_args=None, load_dataset=True):
        ...
        if load_dataset:
            ...
            #     )
            if self.args.test_split:
                # —— 新增：TSPLIB 切换（目录或 .tsp 文件）
                if getattr(self.args, "use_tsplib", False):
                    self.test_dataset = TSPTSPLIBDataset(
                        path=os.path.join(self.args.test_split),
                        optimal_cost_file=getattr(self.args, "tsplib_opt_file", None),
                        optimal_cost_map=getattr(self.args, "tsplib_opt_map", None),  # ← 新增
                    )
                else:
                    self.test_dataset = TSPGraphDataset(
                        data_file=os.path.join(self.args.test_split),
    ...
    def guided_categorical_denoise_step(
        ...
        with torch.inference_mode(False):
            ...
            if not self.sparse:
                ...
                # ! 增加取整
                ewt = getattr(self.args, "edge_weight_type", 0)
                dis_matrix = apply_tsplib_rounding(dis_matrix, ewt)
    ...
    def categorical_test_step(self, batch, batch_idx, split="test"):
        ...
        # ---- 兼容两种数据源：.txt 稠密/稀疏 vs .tsp(tsplib) ----
        is_tsplib = False
        ewt_code = 0
        ...
        if not self.sparse:
            ...
            if batch[2].dim() == 2:  # adj_matrix 存在（老格式）
                real_batch_idx, points, adj_matrix, gt_tour = batch
                np_points = points.cpu().numpy()[0]
                np_gt_tour = gt_tour.cpu().numpy()[0]
            else:
                # TSPLIB: ewt + 可选 gt_cost
                real_batch_idx, points, ewt_tensor, gt_cost_tensor = batch
                is_tsplib = True
                ewt_code = int(ewt_tensor[0].item())
                np_points = points.cpu().numpy()[0]
                gt_cost_from_label = float(gt_cost_tensor[0].item()) if gt_cost_tensor.numel() > 0 else None
                # 为了复用流程，构造一个“空”adj_matrix（不会真正用到）
                N = np_points.shape[0]
        ...
        # ! 修改：兼容 tsplib 的 edge_weight_type
        tsp_solver = TSPEvaluator(np_points, edge_weight_type=ewt_code if is_tsplib else None)

        if is_tsplib:
            # 没有 gt_tour；如果有最优值标签，则用它直接算 gap；否则仅输出 best cost
            best_costs = [tsp_solver.evaluate(solved_tours[i]) for i in range(solved_tours.shape[0])]
            best_solved_cost = float(np.min(best_costs))
            if gt_cost_from_label is not None:
                gap = (best_solved_cost - gt_cost_from_label) / gt_cost_from_label * 100.0
            else:
                gap = float("nan")
            metrics = {
            ...
            for k, v in metrics.items():
                self.log(k, v, on_epoch=True, sync_dist=True, prog_bar=(k.endswith("solved_cost")))
            return metrics
        else:
            # 老格式：有 gt_tour
