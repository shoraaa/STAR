...
from torch.utils.data import DataLoader as TorchDataLoader  # ← 新增
...
class COMetaModel(pl.LightningModule):
    def __init__(self,
        ...
        #     "per_instance": [],  # list of dict(name, N, optimal, cost, gap, time)
        #     "bucket": {
        #         "[0,1000)": [],
        #         "[1000,10000)": [],
        #         "[10000,100000]": [],
        #     },
        #     "bucket_aug": {  # 预留，与 ICAM 一致；本实现不做 augmentation，保持空
        #         "[0,1000)": [],
        #         "[1000,10000)": [],
        #         "[10000,100000]": [],
        #     },
        ...
        # diffusion/pl_meta_model.py 里，COMetaModel.__init__ 的 self.icam_stats 初始化里新增：
        self.icam_stats = {
            "all_instance_num": 0,
            "all_solved_instance_num": 0,
            "per_instance": [],
            ...
            # 如需将来统计“aug 时间”，可再加 "bucket_time_aug"（当前不做 aug 时间统计）
            "start_time_all": None,
        }

    def on_test_start(self):
        self.icam_stats["all_instance_num"] = 0
        self.icam_stats["all_solved_instance_num"] = 0
        self.icam_stats["per_instance"].clear()
        for k in self.icam_stats["bucket"]:
            self.icam_stats["bucket"][k].clear()
            self.icam_stats["bucket_aug"][k].clear()
            self.icam_stats["bucket_time"][k].clear()   # ← 新增
        self.icam_stats["start_time_all"] = time.time()
        logging.getLogger("root").info("#################  Test Start  #################")

    def on_test_end(self):
        logger = logging.getLogger("root")
        end_time_all = time.time()
        solved = self.icam_stats["all_solved_instance_num"]
        total = self.icam_stats["all_instance_num"]
        total_time = end_time_all - (self.icam_stats["start_time_all"] or end_time_all)
        avg_time = (total_time / solved) if solved > 0 else 0.0
        logger.info(f"All scale ranges done, solved instance number: {solved}/{total}, total time: {total_time:.2f}s, avg time per instance: {avg_time:.2f}s")

        # 桶统计输出
        def _avg(x): return float(np.mean(x)) if len(x) else 0.0
        # b = self.icam_stats["bucket"]
        # logger.info("[0, 1000), number: %d, avg gap(no aug): %.3f%%, avg gap(aug): %.3f%%" % (len(b["[0,1000)"]), _avg(b["[0,1000)"]), _avg(self.icam_stats["bucket_aug"]["[0,1000)"])))
        # logger.info("[1000, 10000), number: %d, avg gap(no aug): %.3f%%, avg gap(aug): %.3f%%" % (len(b["[1000,10000)"]), _avg(b["[1000,10000)"]), _avg(self.icam_stats["bucket_aug"]["[1000,10000)"])))
        # logger.info("[10000, 100000], number: %d, avg gap(no aug): %.3f%%, avg gap(aug): %.3f%%" % (len(b["[10000,100000]"]), _avg(b["[10000,100000]"]), _avg(self.icam_stats["bucket_aug"]["[10000,100000]"])))

        b = self.icam_stats["bucket"]
        bt = self.icam_stats["bucket_time"]  # ← 新增
        logger.info("[0, 1000), number: %d, avg gap(no aug): %.3f%%, avg time: %.2fs"
                    % (len(b["[0,1000)"]), _avg(b["[0,1000)"]), _avg(bt["[0,1000)"])))
        logger.info("[1000, 10000), number: %d, avg gap(no aug): %.3f%%, avg time: %.2fs"
                    % (len(b["[1000,10000)"]), _avg(b["[1000,10000)"]), _avg(bt["[1000,10000)"])))
        logger.info("[10000, 100000], number: %d, avg gap(no aug): %.3f%%, avg time: %.2fs"
                    % (len(b["[10000,100000]"]), _avg(b["[10000,100000]"]), _avg(bt["[10000,100000]"])))
        logger.info("#################  All Done  #################")

        # 所有成功实例的平均 gap（无 aug）
        all_gaps = [it["gap"] for it in self.icam_stats["per_instance"] if it.get("gap") is not None]
        logger.info("All solved instances, number: %d, avg gap(no aug): %.3f%%, avg gap(aug): %.3f%%"
                    % (len(all_gaps), _avg(all_gaps), 0.0))

        # 明细列表
        if getattr(self.args, "detailed_log", True):
            names = [it["name"] for it in self.icam_stats["per_instance"]]
            optimals = [it["optimal"] for it in self.icam_stats["per_instance"]]
            sizes = [it["N"] for it in self.icam_stats["per_instance"]]
            costs = [it["cost"] for it in self.icam_stats["per_instance"]]
            gaps = [it["gap"] for it in self.icam_stats["per_instance"]]
            logger.info("instance: %s" % names)
            logger.info("optimal: %s" % optimals)
            logger.info("problem_size: %s" % sizes)
            logger.info("no_aug_score: %s" % costs)
            logger.info("no_aug_gap: %s" % gaps)
            # 可扩展 aug_score / aug_gap 列表
            if len(sizes):
                logger.info("Solved_ instances number: %d, min_dimension: %d, max_dimension: %d, avg gap(no aug): %.3f%%, avg_gap(aug): %.3f%%"
                            % (len(sizes), int(min(sizes)), int(max(sizes)), _avg([g for g in gaps if g is not None]), 0.0))
                logger.info("Avg time per instance: %.2f%s" % (avg_time, "s"))

    # ! 注释以前的 epoch_end 方法，避免和 pl 的自动日志冲突
    ...
    def test_dataloader(self, batch_size=None):
        ...
        # ! —— TSPLIB 数据集是普通 Tensor 元组，走标准 DataLoader
        if getattr(self.args, "use_tsplib", False):
            test_dataloader = TorchDataLoader(self.test_dataset, batch_size=batch_size, shuffle=False)
        else:
