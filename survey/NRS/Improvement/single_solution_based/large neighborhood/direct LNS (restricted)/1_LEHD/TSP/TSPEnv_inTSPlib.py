...
class TSPEnv:
        ...
        with open(filename, 'r') as f:
            ...
            for line in f:
                ...
                if line.startswith("EDGE_WEIGHT_TYPE"):
                    ...
                    self.edge_weight_type = edge_type   # ✅ 保存类型
    ...
    def load_problems(self, episode, batch_size, inst_names, inst_problem_sizes,inst_opt_values):
        ...
        if not self.test_in_tsplib:
            ...
            bb = self.env_params.get(
                'tsplib_path',
                _survey_benchmark_dir("tsp"),
            )
            ...
            self.tsplib_cost = torch.tensor(self.tsplib_cost).float()
    ...
    def sampling_subpaths(self, problems, solution, length_fix=False, mode='test', repair=False):
        ...
        # first_node_index = torch.randint(low=0, high=1000, size=[1])[0]  # in [0,N)
        ...
        if mode == 'test':
            ...
            # ! 限定最高级别
            rrc_range = min(problems_size, 1000)

            length_of_subpath = torch.randint(low=4, high=rrc_range + 1, size=[1])[0]  # in [4,N]
        else:
            if length_fix:
