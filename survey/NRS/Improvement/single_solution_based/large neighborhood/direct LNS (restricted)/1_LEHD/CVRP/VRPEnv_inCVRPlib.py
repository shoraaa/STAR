...
class VRPEnv:
    ...
    def load_problems(self, episode, batch_size, inst_names ):
    ...
    def sampling_subpaths(self, problems, solution, length_fix=False):
        ...
        # length_of_subpath = torch.randint(low=4, high=1000 + 1, size=[1])[0]  # in [4,V]
        ub = np.min([problems_size,1000])
        length_of_subpath = torch.randint(low=4, high=ub + 1, size=[1])[0]  # in [4,V]
    ...
    def sampling_subpaths_repair(self, problems, solution, length_fix=False, mode='test', repair=True):
        ...
        ub = min(1000, problems_size)
        length_of_subpath = torch.randint(low=4, high=ub + 1, size=[1])[0]  # in [4,min(20,N)]
