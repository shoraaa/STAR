...
class VRPModel(nn.Module):
    ...
    def forward(self, state, selected_node_list, solution, current_step, raw_data_capacity=None, point_couples=None, endpoint_mask=None):
        ...
        second_points_data = torch.gather(state.problems[:, :, :2], dim=1, index=second_points_gather_id)
        data_both_point = torch.concat((state.problems[:, :, :3], second_points_data, endpoint_mask.unsqueeze(-1)), dim=2) #[batch, problem_size, 6]

        remaining_capacity = state.problems[:, 1, 3]

        def probs_to_selected_nodes(probs_, split_line_, batch_size_):
            # decide the node to visit and whether to go back to the depot 

            selected_node_student_ = probs_.argmax(dim=1)  # shape: B
            is_via_depot_student_ = selected_node_student_ >= split_line_ # index >= customer_num: back to depot
        ...
        if self.mode == 'train':
            ...
            probs = self.decoder(self.encoder(data_both_point, self.capacity), 
                                 selected_node_list, 
                                 self.capacity, 
                                 remaining_capacity) 
            ...
            loss_node = -prob_select_node.type(torch.float64).log()

        if self.mode == 'test':
            remaining_capacity = state.problems[:, 1, 3]
            if current_step <= 1:
                self.encoded_nodes = self.encoder(data_both_point, self.capacity)
            ...
            is_via_depot_student = selected_node_student >= split_line  # index >= customer_num: via depot
...
class CVRP_Encoder(nn.Module):
    def __init__(self, **model_params):
        ...
        self.embedding = nn.Linear(6, embedding_dim, bias=True) # coord A, demand, coord B, flag

    def forward(self, data_, capacity):
        # data_: coord A, demand, coord B, flag
...
class CVRP_Decoder(nn.Module):
    def __init__(self, **model_params):
        ...
        self.Linear_final = nn.Linear(embedding_dim, 2, bias=False)
    ...
    def forward(self, data, selected_node_list, capacity, remaining_capacity):
        ...
        data_ = data[:, 1:, :].clone().detach() # delete depot
        ...
        embedded_first_node = data[:,[0],:] # depot
        ...
        out = torch.cat((embedded_first_node_,
                         embedded_last_node_), dim=1)
        inter = torch.cat((embedded_first_node_,
                           left_encoded_node_2,embedded_last_node_), dim=1)
        ...
        out[:, [0,-1], :] = out[:, [0,-1], :] + float('-inf')  # first node、depot、last node
        out = torch.cat((out[:, :, 0], out[:, :, 1]), dim=1)  # shape:(B, 2 * ( V - current_step ))

        props = F.softmax(out, dim=-1)  # shape: [B, remind_nodes_number*2+6]
        customer_num = left_encoded_node_2.shape[1]
        props = torch.cat((props[:, 1:customer_num+1], props[:, customer_num+3:-1]), # 2 for depot & last, then 1 for depot
                          dim=1)  # shape:(B,  V - current_step , 2)
