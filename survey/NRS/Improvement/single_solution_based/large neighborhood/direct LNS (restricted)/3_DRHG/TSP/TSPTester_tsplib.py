...
class TSPTester():
    ...
    def run_lib(self, lib_path, scale_ranges=None):
        ...
        for rng in scale_ranges:
            ...
            # ——范围内逐实例——
            smoke_seen = 0
            smoke_limit = int(os.environ.get("NRS_SMOKE_EPISODES", "1")) if os.environ.get("NRS_SMOKE") else None
            stop_smoke = False
            for root, _, files in os.walk(lib_path):
                for file in files:
                    ...
                    if not (rng[0] <= dimension < rng[1]):
                        continue
                    if smoke_limit is not None and smoke_seen >= smoke_limit:
                        stop_smoke = True
                        break
                    smoke_seen += 1
            ...
            for bbbb in range(iter_budget):
                ...
                while not done:
                    if current_step == 0:
                        ...
                        connect_to_another_endpoint = last_is_endpoint & (~last_is_second_endpoint)
                        selected_teacher, _, _, selected_student = tsp_forward_or_model(self.model, state, 
                                                                              self.env.selected_node_list, 
                                                                              self.env.solution,
                                                                              current_step,
                                                                              point_couples=point_couples, 
