...
class TSPEnv:
    ...
    def _get_travel_distance_2(self, problems, solution, test_in_tsplib=False, need_optimal = False):
        ...
        # assert solution.shape[1] == problem_size, "Tour length error: expected Node num {0}, got {1}".format(problem_size, solution.shape[1])
        # # 2. 再检查是否首尾相连
        # assert (solution[:, 0] != solution[:, -1]).all(), "Tour is not a closed loop"
        # batch_size = solution.shape[0]
        # # 3. 检查是否每个节点都访问且仅访问一次
        # for i in range(batch_size):
        #     unique_node_list_len = len(torch.unique(solution[i]))
        #     assert unique_node_list_len == problem_size, \
        #         f"refinement process error, unique_node_list_len:{unique_node_list_len}, problem_size:{problem_size}"
