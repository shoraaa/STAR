...
for _parent in Path(__file__).resolve().parents:
    if (_parent / "NRS" / "native_forward_swap.py").exists():
        ...
        break
from NRS.native_forward_swap import cvrp_forward_or_model
...
class VRPTester():
    def __init__(self,
        ...
        if os.environ.get("NRS_EVAL_SIZE_BUCKET") or os.environ.get("NRS_SMOKE"):
            ...
            keep = (self.inst_problem_sizes >= low) & (self.inst_problem_sizes < high)
            self.inst_names = self.inst_names[keep]
            self.inst_problem_sizes = self.inst_problem_sizes[keep]
            self.inst_opt_values = self.inst_opt_values[keep]
    ...
    def _test_one_batch(self, episode, batch_size, k_nearest, decode_method, clock=None,logger = None):
        ...
        max_memory_allocated_before = _cuda_memory_mb(self.device)
        ...
        with torch.no_grad():
            ...
            if self.env_params['random_insertion']:
                ...
                while not done:
                    ...
                    loss_node, selected_teacher, selected_student, selected_flag_teacher, selected_flag_student = \
                        cvrp_forward_or_model(self.model, state, self.env.selected_node_list, self.env.solution, current_step,
            ...
            max_memory_allocated_after = _cuda_memory_mb(self.device)
