...
class TSPEnv:
    ...
    def TSPLIBReader(self, filename):
        '''
        ...
        with open(filename, 'r') as f:
            ...
            for line in f:
                ...
                if line.startswith("EDGE_WEIGHT_TYPE"):
                    ew_type = line.strip().split(" ")[-1]
                    self.edge_weight_type = ew_type  # ✅ 保存到环境对象
    ...
    def load_problems(self, episode, batch_size, inst_names, inst_problem_sizes,inst_opt_values,
                      problem_size_type=100, dataset_size = None,
        ...
        if self.test_in_tsplib:
            ...
            filename = f'{self.env_params.get("tsplib_path", "/home/shora/Research/NRS_Survey/NRS/0_data_survey/survey_bench_tsp")}/{inst_names}.tsp'
            ...
            locs = np.array(locs)  # 每一行的数据表示一个instance，每一个instance的size不一样
            inst_opt_values = np.array([inst_opt_values])
            inst_names = np.array([inst_names])

            self.tsplib_cost = inst_opt_values
            self.tsplib_name = inst_names
            self.tsplib_problems = locs
            ...
            self.problems = torch.from_numpy(self.tsplib_problems.reshape(1,-1,2)).to(device).float()
            ...
            if repair:
                ...
                if self.test_in_tsplib:
                    ...
                    for line in tqdm(open(self.data_path, "r").readlines()[0 + begin_index:episode + begin_index],
                        ...
                        if line[-1]=='\n':
                            tour_nodes = [int(node) - 1 for node in line[line.index('output') + 1:-2]]
                        else:
    ...
    def destroy_solution(self, problem, complete_solution):
        ...
        self.problems, self.solution,first_node_index,length_of_subpath,double_solution = self.sampling_subpaths(
            problem, complete_solution, mode='test',repair=True)
    ...
    def sampling_subpaths_PRC(self, problems, solution, length_sub, first_index_sub):
        ...
        if factor > 250:
            factor = 250
            print('factor',factor,'problems_size',problems_size,'length_of_subpath',length_of_subpath)
    ...
    def _get_travel_distance(self):
        ...
        if self.decode_method == 'greedy' :
            ...
            if self.test_in_tsplib:
                ...
                B_V = self.selected_student_list.shape[0]
                ...
                segment_lengths_shape = segment_lengths.shape
                segment_lengths = segment_lengths.sum(2).sqrt()

                segment_lengths_shape2 = segment_lengths.shape
