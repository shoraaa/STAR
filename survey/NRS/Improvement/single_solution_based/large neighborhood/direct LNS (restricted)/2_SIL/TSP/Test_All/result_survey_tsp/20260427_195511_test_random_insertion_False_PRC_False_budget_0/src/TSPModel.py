...
class TSPModel(nn.Module):
    ...
    def forward(self, state, selected_node_list, solution, current_step,
                select_probability_accumulate=None, decode_method=None,repair = False):

        batch_size_V = state.data.size(0)
        pproblem_size = state.data.size(1)
        if self.mode == 'train':

            probs = self.decoder(self.encoder(state.data), state.data, state.first_node, state.current_node, selected_node_list, current_step, batch_size_V, pproblem_size)
        ...
        if self.mode == 'test':
            if (decode_method == 'greedy' and repair == False):
                ...
                probs = self.decoder(self.encoded_nodes, state.data,state.first_node, state.current_node, selected_node_list,
                                     current_step, batch_size_V, pproblem_size,mode ='test')

                selected_student = probs.argmax(dim=1)
                selected_teacher = selected_student
                prob = 1

            if (decode_method == 'greedy' and repair == True) :
...
class TSP_Decoder(nn.Module):
    ...
    def replace_inter(self,inter_, replace_index, out):

        inter = inter_.clone().detach()
        batch_size = inter.size(0)
        node_number = replace_index.size(1)
        embedding_dim = inter.size(2)

        index_2_ = (replace_index+1).repeat_interleave(repeats=embedding_dim, dim=1)

        index_1_ = torch.arange(batch_size, dtype=torch.long)[:, None].expand(batch_size, index_2_.shape[1])

        index_3_ = torch.arange(embedding_dim)[None, :].repeat(repeats=(batch_size, node_number))

        inter[index_1_, index_2_, index_3_] = out[:,1:-1,:].reshape(batch_size, node_number* embedding_dim)

        return inter

    def forward(self,data, problems, first_node, current_node,selected_node_list, current_step,batch_size_V,problem_size,mode='train'):
        ...
        out = torch.cat((embedded_first_node_.unsqueeze(1), embedded_last_no2_node_.unsqueeze(1),
                         ...
                         embedded_last_no15_node_.unsqueeze(1),  embedded_last_node_.unsqueeze(1)), dim=1)
        ...
        inter = torch.cat((embedded_first_node_.unsqueeze(1), embedded_last_no2_node_.unsqueeze(1),
                           ...
                           embedded_last_no11_node_.unsqueeze(1), embedded_last_no12_node_.unsqueeze(1),
                           embedded_last_no13_node_.unsqueeze(1), embedded_last_no14_node_.unsqueeze(1),
                           embedded_last_no15_node_.unsqueeze(1), embedded_last_node_.unsqueeze(1),
