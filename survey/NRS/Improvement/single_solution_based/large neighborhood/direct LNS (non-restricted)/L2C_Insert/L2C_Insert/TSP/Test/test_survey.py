...
if log_root:
    logger_params['log_file']['filepath'] = os.path.join(log_root, 'l2c_iter_tsp')
...
def main_test(path,args,file_name,use_RRC=None,cuda_num=None):
    ...
    tester_params['test_episodes'] = (
        int(os.environ.get("NRS_SMOKE_EPISODES", "1"))
        if os.environ.get("NRS_SMOKE")
        else test_paras[args.problem_size][1]
    )
    ...
    env_params['tsplib_path'] = os.environ.get(
        'NRS_SURVEY_TSP_DIR',
        '../../../../../../../../../0_data_survey/survey_bench_tsp',
    )
    # env_params['tsplib_path'] = '../../../../../../../../../0_data_survey/survey_bench_only_tsplib'
    ...
    if cuda_num is None and not os.environ.get("NRS_SKIP_SRC_COPY"):
...
if __name__ == "__main__":
    ...
    RRC_budgets = [1] if os.environ.get("NRS_SMOKE") else [1000]
    # RRC_budgets = [3]
    
    RI_inites = [0]
    coords_norms = [0]
    knearest_ifs = [1]
    rrc_ranges = [1] if os.environ.get("NRS_SMOKE") else [200]
    k_nearest_edge_nums = [100]
    k_nearest_scatter_nums = [100]

    test_in_tsplib_if = 1

    file_name = f'RRC_budgets={str(RRC_budgets)}, RI_inites={str(RI_inites)},' \
