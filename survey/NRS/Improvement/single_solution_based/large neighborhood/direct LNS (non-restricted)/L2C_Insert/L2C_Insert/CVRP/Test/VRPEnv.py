...
class VRPEnv:
    ...
    def load_raw_data(self,episode=1000000):
        ...
        if self.env_params['mode'] == 'test':
            ...
            # ! 新增
                ...
                # 兼容两种输入：
                # 1) 目录：包含若干 .vrp（以及对应 .sol）——参考 ICAM 的读取方式，并按 dimension 升序排序
                # 2) 文件：老的聚合文本（make_vrplib_data 原逻辑），保持兼容
                if os.path.isdir(self.vrplib_path):
                    (self.cvrp_node_coords_all,
                     self.cvrp_demands_all,
                     self.cvrp_capacitys_all,
                     self.vrplib_cost_all,
                     self.vrplib_name_all) = self.make_vrplib_data_from_dir(self.vrplib_path,
                                                                            max_instances=episode,
                                                                            require_label=True)
                else:
                    # 旧数据格式：按原有 txt 读取
                    (self.cvrp_node_coords_all,
                     ...
                     self.cvrp_capacitys_all,
                     self.vrplib_cost_all,
                     self.vrplib_name_all) = self.make_vrplib_data(self.vrplib_path, episode)



        print(f'load raw dataset done!', )

        # ! ========== New: 读取单个 .vrp 文件，仿照 ICAM 的 CVRPLIBReader ==========
    def CVRPLIBReader(self, filename):
        """
        读取 CVRPLIB 格式的 .vrp 文件，并尝试在同目录下寻找 .sol 读取最优解作为 label。
        返回: name, dimension(int, 不含depot), locs(list[[x,y],...], 包含depot), demand(list, depot在首位0), capacity(int), cost(float或None)
        若 EDGE_WEIGHT_TYPE 不是 EUC_2D/CEIL_2D，则返回全 None 以跳过。
        """
        ...
        # 基本校验
        if name is None or dimension <= 0 or capacity is None:
            return None, None, None, None, None, None
        # +1 因为 locs 含 depot
        assert len(locs) == dimension + 1, f'coords length mismatch for {filename}'
        return name, dimension, locs, demand, capacity, cost

    # ! ========== New: 从目录批量读取 + 排序 ==========
    def make_vrplib_data_from_dir(self, dirpath, max_instances=None, require_label=True):
        """
        遍历 dirpath 下所有 .vrp 文件，读取 instance（coords/demand/capacity）与 label（.sol 的 Cost），
        并按照 dimension（客户数）从小到大排序。
        返回与 make_vrplib_data 相同的五元组列表: (node_coords, demands, capacitys, costs, names)
        """
        node_coords, demands, capacitys, costs, names, dims = [], [], [], [], [], []
        ...
        for root, _, files in os.walk(dirpath):
            for file in files:
                ...
                name, dimension, locs, demand, capacity, cost = self.CVRPLIBReader(vrp_path)
                if name is None:
                    continue  # 跳过不支持的 EDGE_WEIGHT_TYPE 或解析失败
                if restrict_size and not (size_low <= dimension < size_high):
                    continue
                if require_label and cost is None:
                    # 与 ICAM 一致：没有 .sol/Cost 的样本跳过
                    continue

                # demand 必须含 depot 的 0 在首位；若文件中没有 0 开头，补齐
                if len(demand) == dimension:
                    demand = [0] + demand
                elif len(demand) != dimension + 1:
                    # 数据不合规，跳过
                    continue

                node_coords.append(locs)      # list of [[x,y],...], depot 在第 0 位
                demands.append(demand)        # list of len = dimension+1，depot demand=0
                capacitys.append(capacity)    # int
                costs.append(cost)            # float (label)
                names.append(name)            # str
                dims.append(dimension)        # int(客户数)

                if max_instances is not None and not restrict_size and len(node_coords) >= max_instances:
                    break
            if max_instances is not None and not restrict_size and len(node_coords) >= max_instances:
                break
        ...
        order = np.argsort(np.array(dims))  # 升序
        node_coords = [node_coords[i] for i in order]
        demands     = [demands[i] for i in order]
        capacitys   = [capacitys[i] for i in order]
        costs       = [costs[i] for i in order]
        names       = [names[i] for i in order]
        if max_instances is not None:
            node_coords = node_coords[:max_instances]
            demands = demands[:max_instances]
            capacitys = capacitys[:max_instances]
            costs = costs[:max_instances]
    ...
    def cal_length_2(self, problems, order_node, order_flag):
        ...
        # 每条边的欧式长度
        order_lengths = ((order_loc - flag_loc) ** 2).sum(2).sqrt()
        ...
        roll_lengths = ((roll_loc - flag_loc) ** 2).sum(2).sqrt()

        # 对每条边“先四舍五入，再加和”
        order_lengths_round = torch.floor(order_lengths + 0.5)
        roll_lengths_round  = torch.floor(roll_lengths  + 0.5)

        length = (order_lengths_round + roll_lengths_round).sum(1)
    ...
    def _get_travel_distance_2(self, problems_, solution_, need_optimal =False ):
        ...
        if self.test_in_vrplib:
            if need_optimal:
                ...
                travel_distances = self.cal_length_2(problems, order_node, order_flag)
        else:
