...
# NEW: for TSPLIB reading like ICAM
from L2C_Insert.TSP.Test.LIBUtils import TSPLIBReader, tsplib_cost
import glob
...
class TSPEnv:
    def __init__(self, **env_params):
        ...
        # ! 增加
        self.tsplib_files = None          # NEW: cache .tsp file list (directory mode)
        self.edge_weight_type = 'EUC_2D'  # NEW: default; will be overwritten by TSPLIBReader

    # ! 新增
    def _read_tsplib_dimension(self, filepath: str):
        """只读取 TSPLIB 文件中的 DIMENSION 字段，用于排序；不解析坐标。"""
        with open(filepath, 'r') as f:
            for line in f:
                # 兼容 "DIMENSION: 280" 或 "DIMENSION 280"
                if line.startswith("DIMENSION"):
                    parts = line.replace(':', ' ').strip().split()
                    # e.g. ["DIMENSION", "280"]
                    return int(parts[-1])
        # 如果未找到就返回 None（后续会跳过该文件）
        return None
    ...
    # ! 新增
    def build_tsplib_file_index(self):
        """扫描 tsplib_path，收集 .tsp 文件，并按 DIMENSION 升序排序；只做索引，不加载实例。"""
        path = self.tsplib_path
        if getattr(self, "tsplib_files", None) is not None:
            return  # 已构建过

        if os.path.isdir(path):
            candidates = glob.glob(os.path.join(path, "*.tsp"))
            if len(candidates) == 0:
                raise FileNotFoundError(f"No .tsp files under directory: {path}")

            records = []
            for fp in candidates:
                dim = self._read_tsplib_dimension(fp)
                if dim is not None:
                    records.append((fp, dim))
            if len(records) == 0:
                raise RuntimeError(f"Found .tsp files but none had a readable DIMENSION in: {path}")
            ...
            if os.environ.get("NRS_EVAL_SIZE_BUCKET") or os.environ.get("NRS_SMOKE"):
                ...
                records = [(fp, dim) for fp, dim in records if low <= dim < high]
                if len(records) == 0:
                    raise RuntimeError(f"No TSPLIB files in requested size range [{low}, {high}) under: {path}")

            records.sort(key=lambda x: x[1])
            if os.environ.get("NRS_SMOKE"):
                records = records[:int(os.environ.get("NRS_SMOKE_EPISODES", "1"))]
            self.tsplib_files = [fp for fp, _ in records]
            self.tsplib_dims  = [dim for _, dim in records]  # 可选：调试用
        elif os.path.isfile(path) and path.endswith(".tsp"):
            self.tsplib_files = [path]
            self.tsplib_dims  = None
        else:
            raise FileNotFoundError(f"tsplib_path should be a directory with .tsp files or a .tsp file: {path}")


    # ! 新增
    def load_problems_lib(self, episode, batch_size):
        """
        Load one TSPLIB instance per episode using ICAM-style reader.
        - self.tsplib_path: directory containing .tsp files, or a single .tsp path
        - Uses LIBUtils.TSPLIBReader + tsplib_cost to fetch coords & optimal
        - Min-max normalize coords to [0,1], but we will rescale back when computing distances
        """
        self.episode = episode
        self.batch_size = batch_size

        # 1) resolve .tsp file path
        path = self.tsplib_path
        # if self.tsplib_files is None:
        #     if os.path.isdir(path):
        #         self.tsplib_files = sorted(glob.glob(os.path.join(path, "*.tsp")))
        #         if len(self.tsplib_files) == 0:
        #             raise FileNotFoundError(f"No .tsp files under directory: {path}")
        #     elif os.path.isfile(path) and path.endswith(".tsp"):
        #         self.tsplib_files = [path]
        #     else:
        ...
        if self.tsplib_files is None:
            if os.path.isdir(path):
                ...
                # 缓存文件路径及其维度（可选：方便调试）
                self.tsplib_files = [fp for fp, _ in records]
                self.tsplib_dims  = [dim for _, dim in records]  # 可选字段
        ...
        # safety: clamp index
        idx = min(episode, len(self.tsplib_files)-1)
        tsp_file = self.tsplib_files[idx]

        # 2) read TSPLIB
        name, dim, locs, ew_type = TSPLIBReader(tsp_file)
        if name is None:
            raise ValueError(f"Unsupported EDGE_WEIGHT_TYPE in {tsp_file}")

        # 3) label (optimal) from dict (same as ICAM)
        if name not in tsplib_cost or tsplib_cost[name] is None:
            raise KeyError(f"optimal value of instance {name} not found in tsplib_cost")
        self.tsplib_cost = torch.tensor([float(tsplib_cost[name])], device="cuda" if self.problems is None or (self.problems.is_cuda if self.problems is not None else True) else "cpu")
        self.tsplib_name = name
        self.edge_weight_type = ew_type if ew_type is not None else "EUC_2D"

        # 4) to torch and normalize to [0,1]
        node_xy = torch.tensor(locs, dtype=torch.float32).unsqueeze(0)  # (1,dim,2)
        self.problems = node_xy.cuda().float() if torch.cuda.is_available() else node_xy.float()

        # store min/max for rescale-back in distance computation
        self.problem_max_min = [torch.max(self.problems), torch.min(self.problems)]
        # min-max [0,1]
        self.problems = (self.problems - self.problem_max_min[1]) / (self.problem_max_min[0] - self.problem_max_min[1] + 1e-9)

        # ground-truth permutation isn't used here; set None
        self.solution = None
        self.problem_size = self.problems.shape[1]
    ...
    def _get_travel_distance_2(self, problems, solution, need_optimal=False):
        if self.test_in_tsplib:
            if need_optimal:
                ...
                # NEW: discretize edges if TSPLIB
                if self.test_in_tsplib:
