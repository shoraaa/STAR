...
def tsp_flow_searching_decode(
    ...
    print(f'num_instances: {num_instances}, num_nodes: {num_nodes}')
    ...
    if threads_over_batches is None or threads_over_batches == 1:
        for i in tqdm.tqdm(range(num_instances // batch_size)):
            batch_cost, batch_all_costs = run_batch(i)
            print(f'batch {i} cost mean: {batch_cost:.6f}')
            costs.append(batch_cost)
            all_costs.append(batch_all_costs)
    else:
        ...
        with ThreadPoolExecutor(max_workers=threads_over_batches) as pool:
            for j in tqdm.tqdm(range(num_instances // batch_size // threads_over_batches)):
                ...
                results = list(pool.map(run_batch, range(batch_id_start, batch_id_end)))
                for i, (batch_cost, batch_all_costs) in zip(range(batch_id_start, batch_id_end), results):
                    print(f'batch {i} cost mean: {batch_cost:.6f}')
                    costs.append(batch_cost)
                    all_costs.append(batch_all_costs)
    ...
    print(f'costs.shape: {costs_tensor.shape}')
    if mean_opt_cost is not None:
        opt_costs_tensor = torch.tensor(opt_costs) if opt_costs is not None else None
        if len(opt_costs_tensor) > len(costs_tensor):
            opt_costs_tensor = opt_costs_tensor[:len(costs_tensor)]
        per_gap = (costs_tensor - opt_costs_tensor) / opt_costs_tensor * 100
        print(f'opt cost: {mean_opt_cost:.6f}')
        print(f'per-instance Gap: {per_gap.mean().item():.6f} %')
...
if __name__ == '__main__':
    ...
    if args.data.endswith('.txt'):
        from data.mcl_convert_fmt import use_saved_problems_tsp_txt
        problems, optimal_score = use_saved_problems_tsp_txt(args.data, device='cpu')
        dataset = {
            'coords': problems.numpy(),
            'opt_costs': optimal_score.numpy(),
        }
    else:
    ...
    end_time = time.time()
    print(f'Total decoding time: {end_time - start_time:.2f} seconds, mins: {(end_time - start_time)/60:.2f} mins')
