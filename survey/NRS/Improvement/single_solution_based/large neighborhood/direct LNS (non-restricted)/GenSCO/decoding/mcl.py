...
from lib import mcl_greedy_insert, mis_edges2neighbors, MISBatchedNeighbors
...
def mcl_flow_decode(
    ...
    adjmat_list = [
        edges2adj(edges, num_nodes_padded, dtype=jnp.float16) if num_nodes_padded < 500  else
        edges2adj(edges, num_nodes_padded, dtype=jnp.float16) + edges2adj(np.flip(edges, axis=-1), num_nodes_padded, dtype=jnp.float16)
    ...
    def inference_fn(
        ...
        *, cycles: int,
        ...
        def _single_run(r: int):
            ...
            solutions, sizes = mcl_greedy_insert(
            ...
            for _ in range(cycles):
                ...
                #     False,
                # )
                solutions = np.logical_xor(solutions, whether_flip)         # allow for both 1->0 and 0->1
                if not decode_intermediate:
