...
class kopt_Decoder(nn.Module):
    ...
    def forward(self, problem, h, rec, context2, visited_time, last_action, fixed_action = None, require_entropy = False):    
          ...
          for i in range(problem.k_max):
              ...
              # action.shape = (bs, 1)
              # next_of_new_action.shape = (bs, 1)
              next_of_new_action = rec.gather(1, action)
              action_index[:,i] = action.squeeze(1).clone()
              k_action_left[stopped,i] = action[stopped].squeeze(1).clone()
              k_action_right[~stopped, i-1] = action[~stopped].squeeze(1).clone()
              k_action_left[:,i+1] = next_of_new_action.squeeze(1).clone()
              ...
              mask[stopped, action[stopped].squeeze(1)] = False # allow next k-opt starts immediately
              # if True:#i == problem.k_max - 2: # allow special case: close k-opt at the first selected node
              index_allow_first_node = (~stopped) & (next_of_new_action.squeeze(1) == action_index[:,0])
