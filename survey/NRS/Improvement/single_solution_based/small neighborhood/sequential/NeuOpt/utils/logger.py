...
def log_to_screen(time_used, init_value, best_value, reward, costs_history, search_history, batch_size, T, opt_score=None):
    # reward
    print('\n', '-'*60)
    ...
    if opt_score is not None and opt_score.sum() > 0:
        opt_score = opt_score.to(best_value.device)
        gap = (best_value - opt_score) * 100 / opt_score
        print('Avg gap (%):'.center(35), '{:<10f} +- {:<10f}'.format(
                gap.mean(), torch.std(gap) / math.sqrt(batch_size)))
    else:
        print('Avg gap:'.center(35), 'N/A')
