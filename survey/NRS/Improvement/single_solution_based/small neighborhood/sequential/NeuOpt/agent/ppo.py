...
class PPO:
    ...
    def rollout(self, problem, T, val_m, stall_limit, batch, record = False, show_bar = False,current_start_batch_idx = 0):
        
        assert val_m == 1, "Currently only allow val_m=1 for experiments."
        
        # ! 修改的代码: 处理lib_model
        bs, gs, dim = batch['coordinates'].size()
        if problem.NAME == 'cvrp':
            problem.reset_size(batch["dimension"], self.opts.dummy_rate,batch['capacity'],device = self.opts.device)
            # print(f'CVRP with {problem.size} nodes:{problem.real_size.item()} real nodes and {problem.dummy_size.item()} dummy depot. Do assert:{problem.with_assert}')
            print(f'CVRP with {problem.real_size.item()} nodes and {problem.dummy_size.item()} dummy depots (total {problem.size}).\n', 
              f'Regulation: {problem.with_regular} Bonus: {problem.with_bonus} Do assert: {problem.with_assert}.\n',
              f'MAX {problem.k_max}-opt. Epsilon: {problem.epsilon}. \n')
        else:
            problem.size = gs
            print(f'TSP with {problem.size} nodes. MAX {problem.k_max}-opt. Do assert: {problem.do_assert}')
        for key in batch: 
            if key in ['name', 'num_sample', 'edge_weight_type','dimension','capacity']:
                continue
            batch[key] = batch[key].to(self.opts.device)
            
        self.actor.reset_CPE_size(gs)
        if not self.opts.lib_model:
            batch_optimal_score = batch["optimal_score"][current_start_batch_idx:current_start_batch_idx+bs].unsqueeze(1).repeat(1,val_m).view(-1)
        else:
            batch_optimal_score = batch["optimal_score"].unsqueeze(1).repeat(1,val_m).view(-1)
        # ! end of new code
        ...
        for t in range(T):
            ...
            # ! 修改的代码
            if show_bar:
                # if (t+1) % 100 == 0 or t == self.opts.T_max - 1:
                #     print(f'Step: {t+1}/{self.opts.T_max} | Avg Cost: {obj.mean().item():.4f}')
                    
                if t < 5 or (t+1) % 100 == 0 or t == self.opts.T_max - 1:
                    print(f'Step: {t+1}/{self.opts.T_max} | Avg Cost: {obj[:, 1].mean().item():.4f}, Avg Optimal: {batch_optimal_score.mean().item():.4f}, Optimality Gap: {((obj[:, 1] - batch_optimal_score) / batch_optimal_score).mean().item()*100:.4f}%')
            

        # assert
        best_length = problem.get_costs(batch_aug_same, solution_best, get_context = False, check_full_feasibility = True, lib_model=self.opts.lib_model)
