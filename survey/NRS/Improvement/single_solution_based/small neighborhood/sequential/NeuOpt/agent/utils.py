...
def validate(rank, problem, agent, val_dataset, tb_logger, distributed = False, _id = None):
    ...
    for batch_id, batch in enumerate(val_dataloader):
        if 'optimal_score' in batch:
            opt_score.append(batch['optimal_score'])
        else:
            raise ValueError("No optimal score in the dataset for validation!")
    ...
    r = torch.cat(r, 0)
    if len(opt_score) > 0:
        opt_score = torch.cat(opt_score, 0)
    else:
        raise ValueError("No optimal score in the dataset for validation!")
    ...
    if distributed and opts.distributed:
        ...
        opt_score = gather_tensor_and_concat(opt_score.contiguous().to(bv.device))
