...
def run(opts):
    ...
    scale_range_all = [[0, 1000]] # ! 先跑1K以下确保有结果, 再跑更大规模的, 假设更大规模gap大于100%
    smoke_limit = None
    ...
    for scale_range in scale_range_all:
        ...
        for batch_id, batch in enumerate(val_dataloader):
            ...
            print(f"Instance {name}, dim={dimension}, length={score}, optimal={optimal_score}, gap={gap}%, time={inst_time}s")
...
if __name__ == "__main__":
    # nohup python -u neuopt_survey_lib.py --problem tsp > neuopt_survey_tsplib_aug1_T1K.log 2>&1 &
    # nohup python -u neuopt_survey_lib.py --problem cvrp > neuopt_survey_cvrplib_aug1_T1K.log 2>&1 &

    warnings.filterwarnings("ignore")
    
    model_scale_list = [200, 100, 50, 20]
    if os.environ.get("NRS_SMOKE") == "1":
        model_scale_list = [100]
