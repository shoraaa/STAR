...
class CVRP(object):
    ...
    def __init__(self, p_size, init_val_met = 'random', with_assert = False, DUMMY_RATE = 0.5, k = 5, with_bonus = True, with_regular = True, lib_model=False):
    ...
    # ! 修改的代码
    def reset_size(self, p_size, DUMMY_RATE,capacity,device):
        self.size = int(np.ceil(p_size.item() * (1 + DUMMY_RATE)))  # the number of real nodes plus dummy nodes in cvrp
        self.real_size = p_size.to(device) # the number of real nodes in cvrp
        self.dummy_size = (self.size - self.real_size).to(device)
        # self.capacity = capacity.to(device)
        self.device = device
        # find closest epsilon
        closest_size = min(EPSILON.keys(), key=lambda x: abs(x - p_size.item()))
        self.epsilon = EPSILON[closest_size]
        assert self.real_size + self.dummy_size == self.size
    ...
    def step(self, batch, rec, action, obj, feasible_history, t, weights = 0):
        ...
        # ! 修改的代码
        next_obj, context = self.get_costs(batch, next_state, True,lib_model=self.lib_model)
    ...
    def get_costs(self, batch, rec, get_context = False, check_full_feasibility = False, lib_model=False):
        ...
        if lib_model:
            coor = batch['lib_data']
            coor_next = batch['lib_data'].gather(1, rec.long().unsqueeze(-1).expand(*rec.size(), 2))
            segment_lengths_raw =  (coor  - coor_next).norm(p=2, dim=2)
            segment_lengths_round = torch.floor(segment_lengths_raw + 0.5)
            cost = segment_lengths_round.sum(1)
        else:
    ...
    def preprocessing(self, solutions, batch):
        ...
        for i in range(seq_length):
            ...
            route_plan_visited_time[arange,next_] = (route % self.dummy_size) * int(ratio) + (i+1) % self.size
        ...
        route_plan_0x = (route_plan_visited_time // int(ratio))
        
        out =  (route_plan_0x, # route plan 0xxxxx
                (route_plan_visited_time % int(ratio)), # visited time
    ...
    def make_dataset(*args, **kwargs):
        ...
        if lib_model:
            ...
            DUMMY_RATE = kwargs.get('DUMMY_RATE')
            return cvrplib_loader(path, scale_range=scale_range, device=device, DUMMY_RATE = DUMMY_RATE)
