...
class CVRP(object):
    ...
    def __init__(self, p_size, init_val_met = 'greedy', with_assert = False, step_method = '2_opt', P = 250, DUMMY_RATE = 0.2, lib_model=False):
    ...
    def get_real_mask(self, rec, batch):
        ...
        if self.step_method == '2_opt':
            ...
            # ! 修改的代码
            mask_btw = ((pi + pj + corj) > (1.+0.1/self.capacity)) | ((qi + qj - corj) > (1.+0.1/self.capacity))
    ...
    def get_costs(self, batch, rec, check_full_feasibility = False, lib_model=False):
        ...
        if lib_model:
            ...
            length = segment_lengths_round.sum(1)
        else:
