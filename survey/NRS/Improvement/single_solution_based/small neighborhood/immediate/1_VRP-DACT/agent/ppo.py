...
class PPO:
    ...
    def rollout(self, problem, val_m, batch, do_sample = False, record = False, show_bar = False,current_start_batch_idx = 0):
        ...
        if problem.NAME == 'cvrp':
            ...
            print(f'CVRP with {problem.size} nodes:{problem.real_size.item()} real nodes and {problem.dummy_size.item()} dummy depot. Do assert:{problem.do_assert}')
        else:
            problem.size = gs
            print(f'TSP with {problem.size} nodes.', ' Do assert:', problem.do_assert)
        for key in batch: 
            if key in ['name', 'num_sample', 'edge_weight_type']:
                continue
        ...
        # get initial cost
        # ! 修改的代码: 处理lib_model
        obj = problem.get_costs(batch, solutions, check_full_feasibility=True, lib_model=self.opts.lib_model)
        # shape: (batch_size * val_m, )
        ...
        for t in range(self.opts.T_max):
        ...
        best_length = problem.get_costs(batch, best_solution, check_full_feasibility=True, lib_model=self.opts.lib_model)
        assert (best_length - obj[:,1] < 1e-5).all(), (best_length, obj, best_length - obj[:,1])

        out = (obj[:,-1].reshape(bs, val_m).min(1)[0], # batch_size, 1
               torch.stack(obj_history,1)[:,:,0].view(bs, val_m, -1).min(1)[0],  # batch_size, T   cost_hist_
               torch.stack(obj_history,1)[:,:,-1].view(bs, val_m, -1).min(1)[0],  # batch_size, T  best_hist_ 
               torch.stack(reward,1).view(bs, val_m, -1).max(1)[0], # batch_size, T
               None if not record else torch.stack(solution_history,1))
        
        return out
    
    # ! self.opts.distributed = False during eval_only
    def start_inference(self, problem, val_dataset, tb_logger):
        if self.opts.distributed:            
            ...
            print("Evaluation with synthetic data.")
