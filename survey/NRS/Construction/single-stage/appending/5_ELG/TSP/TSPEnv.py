...
class TSPEnv:
    ...
    def load_tsplib_problem(self, problems, unscaled_problems, edge_weight_type,aug_factor=1):
        self.tsplib = True
        self.batch_size = problems.size(0)
        self.problem_size = problems.size(1)
        ...
        if aug_factor > 1:
            if aug_factor == 8:
                ...
                raise NotImplementedError
        print("aug_factor in TSPLIB env:", aug_factor)
        print("batch size in TSPLIB env:", self.batch_size)
        print("pomo size in TSPLIB env:", self.pomo_size)
    ...
    def compute_unscaled_distance(self, solutions=None):
        ...
        assert self.unscaled_problems.size(0) == 1, 'The unscaled_problems should be a single instance.'
        assert self.batch_size == 1, 'We do not use augmentation in this survey setting.'
        self.unscaled_problems = self.unscaled_problems.expand(self.batch_size, self.problem_size,2) # batch size is 1 or 8 (aug)
        seq_expanded = self.unscaled_problems[:, None, :, :].expand(self.batch_size, self.pomo_size, self.problem_size, 2)
