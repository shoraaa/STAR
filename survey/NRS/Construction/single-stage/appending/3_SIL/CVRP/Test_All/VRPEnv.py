...
class VRPEnv:
    ...
    def load_problems(self,episode, batch_size,inst_names,problem_size_type=100, dataset_size = None,
        ...
        if not self.test_in_vrplib:
            ...
            self.cvrp_node_coords = locs  # self.cvrp_node_coords_all[episode]
            self.cvrp_demands = demand  # self.cvrp_demands_all[episode]
            self.cvrp_capacitys = capacity  # self.cvrp_capacitys_all[episode]
            self.vrplib_cost = torch.tensor([cost])  # self.vrplib_cost_all[episode]
            self.vrplib_name = name  # self.vrplib_name_all[episode]
            ...
            problem_nodes = torch.from_numpy(problem_nodes).to(device).float()
            ...
            problem_demands = torch.from_numpy(problem_demands).to(device).float()

            capacity_repeat = torch.tensor([problem_capacitys], device=device).float().unsqueeze(0).unsqueeze(1).repeat(1,problem_size,1)
    ...
    def sampling_subpaths_repair_V2(self, problems, solution,length_sub,first_index_sub,
        ...
        # print('start_from_depot', start_from_depot)
        # print('before_start_index_',before_start_index_)


        before_end_index_ = start_from_depot[before_start_index_]

        # print('before_end_index_', before_end_index_)

        before_end_index_[:,:,1] = before_end_index_[:,:,1]


        # print('factor',factor, before_start_indexes[:,[factor-1],:].shape, before_end_index_.shape)
        ...
        # print('before_start_indexes', before_start_indexes,)
    ...
    def random_insert(self,origin_problem):
        ...
        for kk in range(origin_problem.shape[0]):
            ...
            order = None
            # ! 原版这里有问题，进去random insertion里order为None,默认会按照角度排序。已修正为随机打乱顺序
            if self.env_params is not None and self.env_params.get('budget', 1) == 0:
                 # pos is numpy array here, so pos.shape[0] is fine
                order = torch.randperm(pos.shape[0]).cpu().numpy()

            route = cvrp_random_insertion(pos, depotpos, demands, capacity, order=order)
            solution = []
