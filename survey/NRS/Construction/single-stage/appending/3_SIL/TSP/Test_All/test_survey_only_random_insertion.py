...
log_filepath = (
    os.path.join(method_log_root, "random_insertion_tsp", process_start_time.strftime("%Y%m%d_%H%M%S") + "{desc}")
    if method_log_root
    else './result_survey_tsp/' + process_start_time.strftime("%Y%m%d_%H%M%S") + '{desc}'
)
logger_params = {
    'log_file': {
        'desc': 'result_survey',
