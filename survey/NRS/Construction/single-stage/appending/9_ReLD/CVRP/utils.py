...
def augment_xy_data_by_8_fold(problems, aug_idx=-1):
    ...
    # ! 兼容2D输入
    # 兼容 2D 输入： (batch, 2) -> (batch, 1, 2)
    if problems.dim() == 2 and problems.size(-1) == 2:
        problems = problems.unsqueeze(1)
    elif not (problems.dim() == 3 and problems.size(-1) == 2):
        raise ValueError(f"problems shape must be (B, P, 2) or (B, 2), got {tuple(problems.shape)}")
