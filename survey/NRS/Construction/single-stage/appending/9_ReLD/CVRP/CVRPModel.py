    ...
    # ! 补充：捕捉并规范 CUDA OOM 错误，避免测试中断
        ...
        if "out of memory" in msg or isinstance(e, torch.cuda.OutOfMemoryError):
            try:
                ...
                pass
            # 重新抛出为 RuntimeError，包含 "out of memory" 关键词，外层就能接住并跳过实例
            raise RuntimeError(f"CUDA out of memory in multi_head_attention: {e}") from e
        # 其它错误原样抛出
        raise
