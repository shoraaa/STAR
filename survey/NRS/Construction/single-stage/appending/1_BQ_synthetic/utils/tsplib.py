...
def read_tsplib_tsp(filename):
    """
    解析 TSPLIB .tsp 文件，返回：
      name: str
      dimension: int
      coords: np.ndarray [N,2]  (非闭合)
      edge_weight_type: str in {"EUC_2D","CEIL_2D"}
    ...
    with open(filename, "r") as f:
        for line in f:
            ...
            if line.startswith("NAME"):
                ...
                toks = line.replace(":", " ").split()
                ewt = toks[-1]
                if ewt not in {"EUC_2D", "CEIL_2D"}:
                    return None, None, None, None
            elif line.startswith("NODE_COORD_SECTION"):
                started = True
    if name is None or dimension is None or ewt is None:
        return None, None, None, None
    assert len(locs) == dimension, f"TSPLIB parse error: {filename}"
    coords = np.array(locs, dtype=np.float32)
    return name, dimension, coords, ewt


def tsplib_pairwise_dist(coords: np.ndarray, edge_weight_type: str) -> np.ndarray:
    """
    按 TSPLIB 计价规则构造距离矩阵：
      - EUC_2D: 四舍五入的欧氏距离  (通常 round)
      - CEIL_2D: ceil(欧氏距离)
    输入 coords: [N,2] (原始尺度)
    返回 dist: [N,N]  (float32)
    """
    ...
    if edge_weight_type == "CEIL_2D":
        ...
        # TSPLIB 的 EUC_2D 通常是 round 到最近整数，对python来说，应该是floor(x+0.5)
        d = np.floor(d + 0.5)
    else:
        # 理论不会进来
        pass
...
# !!! 把你刚才 LIBUtils.py 里的 tsplib_cost 大字典完整贴过来 !!!
tsplib_cost = {
