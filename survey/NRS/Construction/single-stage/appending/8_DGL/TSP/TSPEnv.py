...
class TSPEnv:
    ...
    def use_saved_problems_tsp_txt(self,filename, total_episodes,device, start=0):
        nodes_coords = []
        solution = []
        for line in tqdm.tqdm(open(filename, "r").readlines()[start:start + total_episodes], ascii=True):
            line = line.split(" ")
        ...
        self.raw_data_nodes = problems
        self.raw_data_tours = solution
        self.baseline_len = optimal_score
