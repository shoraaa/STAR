...
class TSPTester:
    ...
    def run(self):
        ...
        self.aug_size = self.env.aug_size
        beam_size = self.tester_params['beam_size']
        assert beam_size == 1, "beam_size must be 1 for TSP"
        ...
        self.env.use_saved_problems_tsp_txt(self.env_params['data_path'], 
                                            test_episodes_all, 
                                            device=self.device)
        ...
        all_scores = torch.zeros(size=(test_episodes,), dtype=torch.float32).to(self.device)

        with torch.no_grad():
            while episode < test_episodes:
                remaining = test_episodes - episode
                batch_size = min(batch_size, remaining)
                self.env.load_problems(episode,batch_size, problem_size, beam_size)
                
                current_batch_size = self.env.batch_size // self.env.aug_size
                self.batch_size = current_batch_size
                assert current_batch_size == batch_size, "current_batch_size error"

                # Train
                self.env.start_idx = 0
                self.env.step_size = current_batch_size
                # print("self.env.step_size", self.env.step_size)
                while self.env.start_idx < current_batch_size:
                    _, _, best_score,score = self.beamsearch_tour_nodes_shortest(beam_size, self.env.batch_size, self.env.problem_size, 
                                                                    self.dtypeFloat, self.dtypeLong, probs_type='logits', random_start=False)
                    del self.model.dis_matrix
                    torch.cuda.empty_cache()
                    
                    assert current_batch_size == self.env.batch_size // self.env.aug_size, "test_batch_size error"

                    all_scores[episode:episode + self.env.step_size] = score
                    score_AM.update(score.mean().item(), self.env.step_size)
                    best_score_AM.update(best_score, self.env.step_size)

                    self.env.start_idx += self.env.step_size

                batch_baseline_len = self.env.baseline_len[episode:episode + current_batch_size].to(all_scores.device)
                # self.logger.info("batch_baseline: %s", self.env.baseline_len[episode:episode + batch_size])
                
                batch_gap = 100 * (all_scores[episode:episode + current_batch_size] - batch_baseline_len)/batch_baseline_len

                episode += current_batch_size
                

                batch_gap_mean = batch_gap.mean().item()

                gap_AM.update(batch_gap_mean, current_batch_size)

                self.logger.info("episode {:3d}: avg_score {:.4f} batch_gap {:.4f}% avg_gap {:.4f}%".format(episode,score_AM.avg,batch_gap_mean,gap_AM.avg))

        avg_gap = gap_AM.avg
        assert (all_scores > 1.0).all(), "Some scores are invalid!!"
    ...
    def beamsearch_tour_nodes_shortest(self, beam_size, batch_size, num_nodes,
        ...
        while not done:
            ...
            assert selected.shape == (self.batch_size*self.aug_size,), "selected shape error"
        ...
        assert view_reward.shape == (self.batch_size, self.aug_size), "view_reward shape error"
        ...
        assert shortest_lens.shape == (self.batch_size,), "shortest_lens shape error"
        # score = shortest_lens.mean()

        # return keep, best_gap.item(), gap.item(), score.item()
        return 0, 0, 0, shortest_lens
