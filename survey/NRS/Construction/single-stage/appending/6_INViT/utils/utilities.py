...
def calculate_tour_length_by_dist_matrix(dist_matrix, tours,ew_type=None):
    ...
    # ! 添加ew_type处理
    if ew_type == 'CEIL_2D':
        segment_lengths = torch.ceil(dist_matrix[tours, tour_shifts])
        tour_lens = segment_lengths.sum(dim=1)
    elif ew_type == 'EUC_2D':
        segment_lengths = torch.floor(dist_matrix[tours, tour_shifts] + 0.5)
        tour_lens = segment_lengths.sum(dim=1)
    else:
