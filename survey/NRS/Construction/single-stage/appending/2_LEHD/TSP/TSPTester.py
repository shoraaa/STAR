...
class TSPTester():
    ...
    def run(self):
        ...
        while episode < test_num_episode:
            ...
            score_teacher, score_student, gap, problems_size = self._test_one_batch(episode,batch_size,clock=self.time_estimator_2)
            current_gap = gap / 100
            ...
            if all_done:
                ...
                self.logger.info(" Avg. Test Time per Episode: {:.2f} sec ".format(total_time / test_num_episode))
    ...
    def _test_one_batch(self, episode, batch_size,clock=None):
        ...
        with torch.no_grad():
            ...
            return self.optimal_length.mean().item(),current_best_length.mean().item(), gap, self.env.problem_size
