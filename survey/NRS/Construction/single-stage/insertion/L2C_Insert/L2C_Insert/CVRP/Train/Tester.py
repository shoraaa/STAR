...
from L2C_Insert.CVRP.Train.VRPModel import VRPModel as Model
...
class VRPTester():
    ...
    def decide_whether_to_repair_solution(self,
        ...
        need_to_repari_double_solution[x5[if_repair].unsqueeze(2).repeat(1, 1, 2)] = after_repair_sub_solution[
            if_repair].ravel()
        ...
        after_repair_complete_solution = double_solution[x8.unsqueeze(2).repeat(1, 1, 2)].reshape(batch_size,
                                                                                                  the_whole_problem_size,
                                                                                                  -1)
    ...
    def _test_one_batch(self, episode, batch_size, clock=None, logger=None):
        ...
        with torch.no_grad():
            ...
            self.origin_problem_size = self.env.solution.shape[1]
            ...
            while current_step<self.origin_problem_size :
                ...
                if current_step == 0:
                    ...
                    random_index = ploar_distance(scatter_node_coors[:, :, :2], state.data[:,[0],:2],
                                                  partial_end_node_coor[:, :, :2]).reshape(batch_size, 1)
                    # 应该是要计算每条subtour的 remaining capacity
                    # 然后给相应 subtour 的点进行remaining capacity的信息融合？
                    # 把超出capacity的subtour对应的点在最后一层softmax时进行mask？

                    # 先按照remaining capacity给每条subtour的边加mask，后续再考虑把remaining capacity的特征融入整条subtour的边
                    # 如果该subtour的remaining capacity超过了current node的demand，则把这一整条subtour的边mask掉
