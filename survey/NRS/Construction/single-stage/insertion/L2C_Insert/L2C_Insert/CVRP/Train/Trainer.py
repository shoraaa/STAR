...
from L2C_Insert.CVRP.Train.test import main_test
from L2C_Insert.CVRP.Train.VRPEnv import VRPEnv as Env
...
class VRPTrainer:
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.trainer_params['epochs'] + 1):
            ...
            self.logger.info(
                "Epoch {:3d}/{:3d}: Time Est.: Elapsed[{}], Remain[{}]".format(epoch, self.trainer_params['epochs'],
    ...
    def _train_one_batch(self, episode, batch_size, epoch):
        ...
        last_node_index = self.env.abs_partial_solu_2[:, [0], 0]
        ...
        while self.env.abs_scatter_solu_1.shape[1] >= 1:

            partial_end_node_coor = get_encoding(state.data, last_node_index)

            scatter_node_coors = get_encoding(state.data, self.env.abs_scatter_solu_1)

            random_index = ploar_distance(scatter_node_coors[:, :, :2], state.data[:, [0], :2],
                                          partial_end_node_coor[:, :, :2]).reshape(batch_size, 1)


            remaining_capacity = cal_remaining_capacity(self.origin_problem, self.env.abs_partial_solu_2, capacity=raw_capacity)

            prob, expanded_padding_partial_solution, abs_scatter_solu_1_unseleted, abs_scatter_solu_1_seleted = \
            ...
            self.env.abs_scatter_solu_1 = abs_scatter_solu_1_unseleted

            self.env.abs_partial_solu_2 = tran_to_node_flag(expanded_padding_partial_solution)
        ...
        loss_mean_ = - prob_list.log().mean()

        return 0, 0, loss_mean_.item()
