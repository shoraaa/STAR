...
def use_saved_problems_tsp_txt(filename, total_episodes=10000,device='cpu', start=0):
    nodes_coords = []
    solution = []
...
class TSPDataset(Dataset):
    def __init__(self, size=100, node_dim=2, num_samples=10000, offset=0, data_distribution="uniform", data_path=None):
        super(TSPDataset, self).__init__()
        ...
        if data_path is not None:
            if os.path.splitext(data_path)[1] == '.pkl':
                ...
                problems, optimal_score = use_saved_problems_tsp_txt(data_path, num_samples, 'cpu', start=offset)
                self.data = [row for row in problems]
                self.optimal_score = optimal_score
            else:
                assert os.path.splitext(data_path)[1] == '.pkl'
        else:
            raise ValueError("A filename must be provided for TSPDataset.")
